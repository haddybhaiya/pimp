...
2. **Zero Secret & PII Leakage (INV-DISC-02):**  
   Public merchant discovery profiles and search responses must never expose Razorpay keys, webhook secrets, session tokens, database IDs/credentials, private policies, floor prices, internal margins, audit logs, or customer PII. Strict allowlisting is enforced.
...
5. **Deterministic Matching & Integer Budget Safety (INV-DISC-05):**  
   Buyer intent evaluation, capability filtering, delivery region matching, and price range checks are completely deterministic. Budget checks enforce integer multiplication overflow guards (`min_var_paise * qty <= maximum_budget_paise`). Unsupported capabilities and non-deliverable regions fail closed.
