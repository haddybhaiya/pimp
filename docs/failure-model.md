...
| **Hidden Structured Prohibited Action** | An LLM places prohibited intent beneath a neutral metadata label | Deterministic governance screens every untrusted scalar structured value | Rejects the proposal as `PROHIBITED` before persistence | Neutral key names cannot downgrade financial/governance risk. |
...
| **Late Conversion Settlement Bias** | A quote settles after its observation endpoint | Conversion cohorts include only orders settled by each matching endpoint | Later settlement affects a future observation, not a historic experiment result | Baseline and post windows have matching settlement maturity. |
