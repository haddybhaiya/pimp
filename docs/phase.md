...
- [x] **Database Models & Alembic Migrations 011–012 (`011_phase8_controlled_autonomy.py`, `012_autonomy_proposal_tenant_integrity.py`):**
  ...
  - Created `merchant_autonomy_actions` ledger with snapshot storage, composite foreign keys to `merchant_experiments(id, merchant_id)` and `merchant_proposals(id, merchant_id)`, and idempotency indexing.
- [x] **Authoritative 18-Precondition Gate Pipeline (`ControlledAutonomyService.execute_autonomous_action`):**
  ...
  - Product mutations require an explicitly identified tenant-owned product; targetless proposals fail closed rather than selecting an arbitrary catalog row.
